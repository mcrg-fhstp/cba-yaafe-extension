...
def listFeatures():
    features = [feat.__name__ for feat in yaafe.AudioFeatureFactory.get_all_features() if not feat.TRANSFORM]
    transforms = [feat.__name__ for feat in yaafe.AudioFeatureFactory.get_all_features() if feat.TRANSFORM]
    if len(features)==0:
...
def showFeatures(h5file):
    ...
    for name,fd in fdict.iteritems():
        print '%s [%ix%i] %iHz:'%(name,fd['dim'],fd['nbframes'],fd['sampleRate'])
...
def main():
	...
	### Adaptions for CBA
        ...
        for audiofile in audiofiles:
            if afp.processFile(engine,audiofile) != 0:
                return 1
            feats=engine.readAllOutputs()	# output values directly to shell
            import numpy as np				# import numpy for next command
            np.set_printoptions(threshold='nan')	# disable cropping of large arrays
            import json
            print json.dumps(feats,cls=NumpyAwareJSONEncoder)


# Appendix for json output of numpy-arrays
# found: http://stackoverflow.com/questions/3488934/simplejson-and-numpy-array
import json
import numpy
